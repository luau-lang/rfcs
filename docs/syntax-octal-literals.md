...
Unix also uses octal to represent file permissions (e.g. in commands like `chmod 755`), and this is perhaps the most recognizable use of it today.
...
Unlike `0B` and `0X`, `0O` is not accepted as a valid prefix. This exception is made to avoid problems with octal literal readability.
